...
class RaidAvgDPSOverlay(BaseOverlay):
	def __init__(self):
		BaseOverlay.__init__(self, title="Raid Avg. DPS", size=(300, 150))
	...
	def createUI(self):
		...
		self.grid.SetColLabelValue(1, "DPS")
	...
	def OnAnalyzerTick(self, analyzer):
		...
		for player in sorted(raid.playerData, sortf):
			...
			self.grid.SetCellValue(index, 1, locale.format("%.2f", avgDps, grouping=True))

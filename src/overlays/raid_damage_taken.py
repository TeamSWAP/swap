...
class RaidDamageTakenOverlay(BaseListOverlay):
	...
	def createUI(self):
		...
		self.setColumns(["Player", "Damage Taken", "%"], [0.4, 0.4, 0.2], [BaseListOverlay.LEFT, BaseListOverlay.LEFT, BaseListOverlay.RIGHT])
	...
	def OnAnalyzerTick(self, analyzer):
		...
		for player in sorted(raid.playerData, sortf):
			...
			self.addRow([player['name'][1:], locale.format("%d", player['totalDamageTaken'], grouping=True), "%s%%"%percent], color)

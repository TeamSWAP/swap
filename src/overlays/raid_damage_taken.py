...
class RaidDamageTakenOverlay(BaseOverlay):
	def __init__(self):
		BaseOverlay.__init__(self, title="Raid Damage Taken", size=(300, 150))
	...
	def createUI(self):
		...
		self.grid.SetColLabelValue(1, "Damage Taken")
	...
	def OnAnalyzerTick(self, analyzer):
		...
		for player in sorted(raid.playerData, lambda x: x['totalDamageTaken']):
			if raidTotalDamageTaken > 0:
				percent = "%.2f"%((player['totalDamageTaken'] / raidTotalDamageTaken) * 100.0)
			else:
				percent = "%.2f"%0
			...
			self.grid.SetCellValue(index, 1, locale.format("%d", player['totalDamageTaken'], grouping=True))

...
class RaidHealingReceivedOverlay(BaseOverlay):
	def __init__(self):
		BaseOverlay.__init__(self, title="Raid Healing Received", size=(300, 150))
	...
	def createUI(self):
		...
		self.grid.SetColLabelValue(1, "Healing Received")
	...
	def OnAnalyzerTick(self, analyzer):
		...
		for player in sorted(raid.playerData, lambda x: x['totalHealingReceived']):
			if raidTotalHealingReceived > 0:
				percent = "%.2f"%((player['totalHealingReceived'] / raidTotalHealingReceived) * 100.0)
			else:
				percent = "%.2f"%0
			...
			self.grid.SetCellValue(index, 1, locale.format("%d", player['totalHealingReceived'], grouping=True))
